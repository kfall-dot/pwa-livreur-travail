...
export function isProcurementWorkspaceRole(role: string | null | undefined): boolean {
  return role === 'technical_director' || role === 'purchasing' || role === 'daf' || role === 'pdg'
}
...
export function canCreatePo(
  ...
  return role === 'purchasing' && (status === 'sa_review' || status === 'bt_pending')
}
...
export function canScheduleDelivery(
  ...
  if (status === 'po_ready') return true
  return status === 'delivery_scheduled' && !hasLinkedTour
}
