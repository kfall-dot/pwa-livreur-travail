...
<head>
  ...
  <style>
    ...
    .sign .box { height: 72px; white-space: pre-line; }
...
<body>
  <div class="fiche">
    ...
    <table class="sign">
      <tr>
        <td class="head" style="width:16%">TRAITE PAR</td>
        <td class="head">NOM</td>
        <td class="head">DATE</td>
        <td class="head">SIGNATURE</td>
        <td class="head" rowspan="2" style="width:22%">DAF</td>
      </tr>
      <tr>
        <td></td>
      ...
      <tr>
        <td class="head">VALIDE PAR</td>
        <td class="head">NOM</td>
        <td class="head">DATE</td>
        <td class="head">SIGNATURE</td>
        <td class="head">DAF</td>
      </tr>
      <tr>
        <td></td>
...
/** TRAITE PAR = SA ; VALIDE PAR = DT — même bloc sur fiche DT et fiche SA. */
export function signoffFromApprovalSteps(
  steps: Array<{ role: string; decision: string; comment: string | null; createdAt: Date | string }>,
): Pick<
  EbFicheData,
  | 'treatedByName'
  | 'treatedByDate'
...
> {
  ...
  return {
    treatedByName: firstLineName(sa?.comment),
    treatedByDate: sa ? fmt(sa.createdAt) : '',
    treatedBySignature: sa?.comment ?? '',
    validatedByName: firstLineName(dt?.comment),
    validatedByDate: dt ? fmt(dt.createdAt) : '',
    validatedBySignature: dt?.comment ?? '',
  }
}
