...
| **Version**          | 1.1 — août 2026 (intégration questionnaire finance + règle BT)                                     |
| **Périmètre**        | Approvisionnement matériaux chantier BTP — pilote TraceO Achats Chantier                           |
| **Public**           | Board des gestionnaires, DT, DAF, SA, PDG, direction exploitation                                  |
| **Plateforme cible** | TraceO® — extension du module livraison existant                                                   |
| **Statut**           | Document de référence — décisions DT D1–D4 validées ; validation DAF/SA/board en cours             |
| **Sources**          | Fiches SA (`docs/originaux/`), entretien DT (août 2026), prototype TraceO local, règles métier BTP |
...
La solution s’appuie sur le **moteur tournées/livreur existant** ; ce n’est pas un ERP ni un second logiciel.
...
| Inclus                                               | Exclu (phase 1)                                |
| ---------------------------------------------------- | ---------------------------------------------- |
| Expression de besoin (EB) — déboursé sec et courante | ERP comptable complet                          |
| Circuit approbation DAF / BT / PDG / SA              | RH, paie                                       |
| BC, registre fournisseurs, exports Excel SA          | Planification ouvrages (Gantt, BIM)            |
| Tournées livreur liées au BC                         | Lecture passive intégrale des groupes WhatsApp |
| Preuve livraison, alertes partielles                 | Multi-dépôts, valorisation FIFO                |
| Stock chantier (phases progressives)                 | Transferts stock inter-chantiers               |
...
| **Prototype TraceO** (local, non déployé)           | Faisabilité technique du TO-BE                 |
...
| Dashboard manager      | Opérationnel    | Ajouter onglet **Achats chantier**, **Stock**, **Dossier chantier** |
| Extension BTP          | Prototype local | Déployer en pilote après validation board                           |
...
**Ce que la recommandation n’est pas :** un ERP, un remplacement WhatsApp, une commande automatique sans validation DT.

### 4.6 Choix stratégiques argumentés
...
| **2b** | +4 sem.   | Bilan soir WhatsApp → OUT ; EB suggérée sous seuil                    | Anticipation             |
| **3**  | +4 sem.   | Dossier chantier, jalons évolution, galerie photos                    | Vue direction            |
