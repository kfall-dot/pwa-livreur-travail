...
describe('ebSignature (signature_simple)', () => {
  ...
  it('refuse une étape hors rôle (DT ≠ validation_pdg)', () => {
    ...
    assert.equal(assertEtapeForRole('CDG', 'validation_cdg'), null)
    assert.match(assertEtapeForRole('CDG', 'approbation_daf_1') ?? '', /non autorisée/)
  })
  ...
  it('mappe le rôle achats vers DT/SA/DAF/PDG', () => {
    ...
    assert.equal(procurementRoleToSignatureRole('controle_gestion'), 'CDG')
  })

  it('formate le bloc signature avec NIP vérifié', () => {
    ...
    assert.match(block, /NIP vérifié/)
